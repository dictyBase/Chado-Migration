...
SQL::Translator::Producer::OraclePlus - Oracle SQL producer with diffing of SQL enabled
...
sub produce {
    ...
    for my $table ( $schema->get_tables ) {
        my ( $table_def, $fk_def, $trigger_def, $index_def, $constraint_def )
            = create_table(
            $table,
    ...
    if (wantarray) {
        ...
        $create .= join( ";\n\n",
            @table_defs, @view_defs, @fk_defs, @index_defs,
            @constraint_defs );
        $create .= ";\n\n";
...
sub create_table {
    ...
    for my $c ( $table->get_constraints ) {
        ...
        if ( $c->type eq PRIMARY_KEY ) {
            ...
            push @fk_defs,
                sprintf( "ALTER TABLE %s ADD %s", $table_name_q, $def );
        }
    }
    ...
    if ( my @table_comments = $table->comments ) {
        for my $comment (@table_comments) {
            ...
            $comment =~ s/'/''/g;
            push @field_comments,
                "COMMENT ON TABLE $table_name_q is\n '" . $comment . "'"
                unless $options->{no_comments};
        }
    }
    ...
    @constraint_defs
        = map {"ALTER TABLE $table_name_q ADD $_"} @constraint_defs;
...
sub create_field {
    ...
    if ( $field->is_auto_increment ) {
        ...
        push @create, qq[DROP SEQUENCE $seq_name]
            if $options->{add_drop_table};
