    ...
    sub {
        ...
        my @files = File::Find::Rule->file->name(qr/pubmed/)
            ->in( $dir->subdir('release-2-20')->stringify );
    ...
    sub load_entry {
        ...
        while ( my $entry = $biblio->next_bibref ) {
            ...
            if (my $exist
                = Modware::Publication::DictyBase->find_by_pubmed_id(
                    $pubmed_id)
                )
            {
            ...
            $pub->abstract( $entry->abstract ) if $entry->abstract;
            ...
            for my $pub_author ( @{ $entry->authors } ) {
                ...
                for my $slot (qw/initials suffix/) {
                    if ( $pub_author->can($slot) and $pub_author->$slot ) {
                        $author->$slot( $pub_author->$slot );
                    }
                }
